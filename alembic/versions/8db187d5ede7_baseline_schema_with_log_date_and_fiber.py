...
def downgrade() -> None:
    ...
    Alembic's autogenerated version dropped each index before its table. That
    is unnecessary -- DROP TABLE removes a table's indexes with it -- and it
    fails on SQLite, where batch mode recreates the table and the composite
    index is already gone by the time the drop is attempted. Tables are dropped
